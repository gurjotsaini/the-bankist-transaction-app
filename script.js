...
const startLogoutTimer = function () {
    const tick = function () {
        ...
        // In each call, prime the timer with the remaining time
        labelTimer.textContent = `${min}m ${sec}s`;

        // When timer is finished, logout
        if (time === 0) {
            clearInterval(timer);
...
labelBalance.addEventListener('click', function () {
    [...document.querySelectorAll('.movements__row')].forEach(function (
        ...
        if (i % 2 === 0) row.style.backgroundColor = '#f2f2f2';
        // if (i % 3 === 0) row.style.backgroundColor = '#fffeee';
    });
});
